...
> {
  ...
  return (async function* () {
    for await (const chunk of stream) {
      ...
      if (!delta) continue;
      const audio = delta.audio as { transcript?: string; data?: string } | undefined;
      if (audio?.transcript) {
      ...
      const inputAudio = delta.input_audio as { transcript?: string } | undefined;
      const userFromInput =
        typeof inputAudio?.transcript === "string" ? inputAudio.transcript : "";
      const userFromDelta =
        typeof delta.user_transcript === "string" ? delta.user_transcript : "";
      const userText = userFromInput || userFromDelta;
      if (userText) {
